...
from tf.keras.activations import gelu # add gelu - only available in tf-nightly. (or tfa) # https://www.tensorflow.org/api_docs/python/tf/keras/activations/gelu

def _create_model(x, seq_len, vocab_size, n_annotations,
                  d_hidden_seq=512, d_hidden_global=2048,
                  n_blocks=6, n_heads=8, d_key=64, conv_kernel_size=9, wide_conv_dilation_rate=5,activation="relu"):
    ...
    hidden_seq = keras.layers.Dense(d_hidden_seq, activation=activation, name='dense-seq-input')(encoded_input_seq)
    ...
    for block_index in range(1, n_blocks + 1):
        ...
        narrow_conv_seq = keras.layers.Conv1D(filters=d_hidden_seq, kernel_size=conv_kernel_size, strides=1, padding='same', dilation_rate=1, activation=activation, name='narrow-conv-block%d' % block_index)(hidden_seq)
        wide_conv_seq = keras.layers.Conv1D(filters=d_hidden_seq, kernel_size=conv_kernel_size, strides=1, padding='same', dilation_rate=wide_conv_dilation_rate, activation=activation, name='wide-conv-block%d' % block_index)(hidden_seq)
