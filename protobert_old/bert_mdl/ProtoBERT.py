...
def get_encoded_annotations():
    ...
    print('There are %d unique annotations.' % n_unique_annotations)
    annotation_to_index = {annotation: i for i, annotation in enumerate(unique_annotations)}
    encoded_annotations = np.array([encode_labels(record_annotations, annotation_to_index, n_unique_annotations) for record_annotations in annotations ], dtype='int8')
    print('Shape of encoded annotations: %dx%d' % encoded_annotations.shape)
    return unique_annotations, encoded_annotations
...
def binary_PFA(y_true, y_pred, threshold=K.variable(value=0.5)):
    ...
    # PRETRAINING_N = total number of negative labels
    N = K.sum(1 - y_true)
...
def process_submission_batch(prediction_model, formatted_predictions, batch_cafa_ids, batch_seqs, batch_go_annotation_indices, go_annotation_indices):
    ...
    for cafa_id, annotation_predictions in zip(batch_cafa_ids, batch_annotation_predictions):
        ...
        try:
            ...
            # TODO: FIX MEEEEEE NADAV!!!
            print('\n\n\nSOMETHING IS WRONG WITH IDX\n\n\n')
            traceback.print_exc()
            formatted_predictions.extend(['%s\t%s\t%s' % (cafa_id, 999, score) for \
                                          annotation_index, score in top_predicted_annotations if score > 0])
...
def create_prediction_model(sp, model, unique_annotations):
    ...
    def prediction_model(batch_seqs, batch_go_annotation_indices):
        ...
        batch_encoded_annotations = np.array([encode_labels(go_annotation_indices, annotation_to_index, len(unique_annotations)) for go_annotation_indices in batch_go_annotation_indices])
        ...
        batch_pred_annotation_scores = model.predict([batch_padded_seq_tokens, seg_input, mask_input])
        batch_pred_annotation_scores_as_dicts = [{unique_annotations[i]: score  for i, score in enumerate(pred_annotation_scores)} for pred_annotation_scores in batch_pred_annotation_scores]
...
def run_pipeline():
    ...
    # unique_annotations, encoded_annotations = get_encoded_annotations()
